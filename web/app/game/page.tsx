...
export default function GamePage() {
  ...
  useEffect(() => {
    if (computerBoxes.length > 0 && !gameEnded && gameStartTime) {
      runComputerSort();
    }
  }, [computerBoxes.length, gameEnded, gameStartTime]);

  const runComputerSort = async () => {
    const values = computerBoxes.map(b => b.value);
    let stopped = false;
      ...
      <div className="sorting-container" id="gameContainer" style={{ display: 'block' }}>
        <div className="sorting">
          ...
          <div className="computer">
            {computerBoxes.length > 0 ? (
              computerBoxes.map((box, index) => (
                <SortingBoxComponent key={`computer-${index}-${box.value}`} box={box} isComputer />
              ))
            ) : (
              <p style={{ gridColumn: '1 / -1', textAlign: 'center' }}>Lädt...</p>
            )}
          </div>
